...
def metric_func(output_dict, label_dict):
    logger.info(f"mean of lambda: {output_dict['lambda_'].numpy().mean()}")
    logger.info(f"mean of mu: {output_dict['mu'].numpy().mean()}")
    return ppsci.metric.MAE()(output_dict, label_dict)
...
def train(cfg: DictConfig):
    ...
    lr_scheduler = None
    optimizer = ppsci.optimizer.LBFGS(**cfg.TRAIN.opt)(
        (inverse_lambda_net, inverse_mu_net)
    )
    ...
    geom_validator = ppsci.validate.GeometryValidator(
        ...
        ppsci.loss.L1Loss("mean"),
        metric={"MAE": ppsci.metric.FunctionalMetric(metric_func)},
        name="geo_eval",
    )
