...
import paddle
import sympy
from loss import FunctionalLossBatch
...
class Trainer:
    def __init__(self, solver: "Solver", is_batch: bool = False) -> None:
        self.solver = solver
    ...
    def train_nonbatch(self):
        for iter_id in range(1, self.solver.iters_per_epoch + 1):
            ...
            for _, _constraint in self.solver.constraint.items():
                ...
                self.loss_dict["loss"].append(float(total_loss))
    ...
    def train_backward_batch(self, constraint_losses_list):
        for iter_id in range(1, self.solver.iters_per_epoch + 1):
            # compute loss for each constraint according to its' own output, label and weight
            total_loss = 0
    ...
    def train(self):
        ...
        for epoch_id in range(
            ...
            self.loss_dict = misc.Prettydefaultdict(list)
            self.loss_dict["loss"] = []
            if self.is_batch:
                # forward
                (
                ...
                # backward
                self.train_backward_batch(constraint_losses_list)
            else:
                self.train_nonbatch()

            # log training summation at end of a epoch
            logger.info(
                f"[Train][Epoch {epoch_id}/{self.solver.epochs}][Avg] loss: {np.mean(self.loss_dict['loss'])}"
            )
            ...
            save_load.save_checkpoint(
                ...
                metric={"metric": cur_metric, "epoch": epoch_id},
                grad_scaler=self.solver.scaler,
                output_dir=self.solver.output_dir,
                prefix="latest",
                equation=self.solver.equation,
                print_log=False,
            )
...
class StratifiedInteriorConstraint(base.Constraint):
    def __init__(
        ...
        inputs, mask = sampler.sample_interior_stratified(
        ...
        for key, value in label_dict.items():
            ...
            if isinstance(value, (int, float)):
                label[key] = np.full_like(next(iter(inputs.values())), value)
                ...
                label[key] = func(
                    **{k: v for k, v in inputs.items() if k in geom.dim_keys}
                )
            elif callable(value):
                func = value
                label[key] = func(inputs)
                if isinstance(label[key], (int, float)):
                    label[key] = np.full_like(next(iter(inputs.values())), label[key])
            else:
        ...
        dataloader_cfg["dataset"].update(
            {"input": inputs, "label": label, "weight": weight}
        )
