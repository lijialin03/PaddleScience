...
def train(cfg: DictConfig):
    ...
    # set problem
    problem = getattr(problems_module, cfg.PROBLEM["name"])(cfg.PROBLEM)
    ...
    #     loss_module.FunctionalLoss(problem.disp_loss_func),
    #     bounds=(
    ...
    #     },
    #     # ppsci.loss.MSELoss(loss_str),
    #     loss_module.FunctionalLossBatch(problem.density_loss_func),
    #     bounds=(
    #         (problem.geo_origin[0], problem.geo_dim[0]),
    #         (problem.geo_origin[1], problem.geo_dim[1]),
    #     ),
    ...
    interior_density = ppsci.constraint.SupervisedConstraint(
        {
            "dataset": {"name": "NamedArrayDataset", "input": inputs},
            "sampler": {
                "name": "DistributedBatchSampler"
                if cfg.TRAIN.enable_parallel
                else "BatchSampler",
    ...
    disp_trainer = func_module.Trainer(solver_disp)
    density_trainer = func_module.Trainer(solver_density, is_batch=problem.use_mmse)

    # training
    disp_trainer.train()

    for i in range(1, cfg.TRAIN.epochs + 1):
        logger.info(f"[Total Train] Epoch: {i}")
        ...
        if i == 1 or i % cfg.log_freq == 0 or i == cfg.TRAIN.epochs:
            ...
            save_load.save_checkpoint(
                model=solver_density.model,
                optimizer=solver_density.optimizer,
                grad_scaler=solver_density.scaler,
                metric={"metric": "dummy", "epoch": i},
                output_dir=solver_density.output_dir,
                prefix=f"epoch_{i}",
                equation=solver_density.equation,
            )
